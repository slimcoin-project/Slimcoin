...
bool CBlock::AcceptBlock()
{
    ...
    if ((IsProofOfStake() && (pindexPrev->nHeight >= POS_HARDCAP_HEIGHT)) && (GetPoSRowCount(pindexPrev) >= POS_HARDCAP))
...
CBlock *CreateNewBlock(CWallet* pwallet, bool fProofOfStake, const CWalletTx *burnWalletTx, CReserveKey *resKey)
{
    ...
    static int64 nLastCoinStakeCandidateUpdateTime = 0;  // only initialized at startup
    ...
    if (fProofOfStake)  // attemp to find a coinstake
    {
        ...
        if(nSearchTime - nLastCoinStakeCandidateUpdateTime >= 300)
        {
            ...
            nLastCoinStakeCandidateUpdateTime = nSearchTime;           
...
void SlimCoinMiner(CWallet *pwallet, bool fProofOfStake)
{
    ...
    while (fGenerateSlimcoins || fProofOfStake)
    {
        ...
        if ((fProofOfStake && pindexPrev->nHeight >= POS_HARDCAP_HEIGHT) && GetPoSRowCount(pindexPrev) >= POS_HARDCAP)
            return;
