...
bool CWallet::CreateStealthTransaction(CScript scriptPubKey, int64_t nValue, std::vector<uint8_t>& P, CWalletTx& wtxNew, CReserveKey& reservekey, int64& nFeeRet)
{
    vector< pair<CScript, int64> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
            ...
            for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
            {
                ...
                */
                ...
                if (IsLocked())
                {
                    ...
                    AddCryptedKey(cpkE.Raw(), vchEmpty);
